...
namespace Petit.Runtime
{
    class TestRun_Assign : TestRunBase
    {
        ...
        {
            {
                ...
                Assert.AreEqual(interpreter.Environment["a"], 10);
            }
            {
                Interpreter interpreter = RunInt("a = b = 10", 10);
                Assert.AreEqual(interpreter.Environment["a"], 10);
                Assert.AreEqual(interpreter.Environment["b"], 10);
            }
            {
                Interpreter interpreter = RunInt("a = b = 1 + 2 * 3", 7);
                Assert.AreEqual(interpreter.Environment["a"], 7);
                Assert.AreEqual(interpreter.Environment["b"], 7);
            }
            {
                Interpreter interpreter = RunInt("a=10; a+=2", 12);
                Assert.AreEqual(interpreter.Environment["a"], 12);
            }
            {
                Interpreter interpreter = RunInt("a=10; b=2; b+=a+a;", 22);
                Assert.AreEqual(interpreter.Environment["a"], 10);
                Assert.AreEqual(interpreter.Environment["b"], 22);
            }
        }
        [Test]
        ...
        {
            {
                ...
                Assert.AreEqual(interpreter.Environment.Get("a"), 10);
                Assert.AreEqual(interpreter.Environment.Get("b"), Value.Invalid);
            }
            {
                Interpreter interpreter = RunInt("(false ? a : b) = 10", 10);
                Assert.AreEqual(interpreter.Environment.Get("a"), Value.Invalid);
                Assert.AreEqual(interpreter.Environment.Get("b"), 10);
            }
        }
        [Test]
        public void TestPrefixIncrement()
        {
            ...
            Assert.AreEqual(interpreter.Environment.Get("a"), 2);
        }
        [Test]
        public void TestPrefixDecrement()
        {
            Interpreter interpreter = RunInt("a=1;--a;", 0);
            Assert.AreEqual(interpreter.Environment.Get("a"), 0);
        }
        [Test]
